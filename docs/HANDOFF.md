...
(→ openssh.org), **libressl.org**, **openbsdfoundation.org**. Settings is
...
1. **This / next session — D.** Claim another static document (not
   Wikipedia / GitHub / mail). Twenty-two public claims; CSS leftovers
   done. Candidates: openbgpd.org, or similar.
...
6. **v0.2.0** when: named static list is claimed (22 already), worker
...
(→ openssh.org), libressl.org, openbsdfoundation.org.

## Next session — start here

**D: claim another static document** (not Wikipedia / GitHub / mail).
Twenty-two public sites (openbsdfoundation.org claimed). Candidates:
openbgpd.org, or similar. Arch package builds; install is `sudo pacman
