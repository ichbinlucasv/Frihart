...
(→ openssh.org), **libressl.org**. Settings is
...
1. **This / next session — D.** Claim another static document (not
   Wikipedia / GitHub / mail). Twenty-one public claims; CSS leftovers
   done. Candidates: openbsdfoundation.org, or similar.
...
6. **v0.2.0** when: named static list is claimed (21 already), worker
...
(→ openssh.org), libressl.org.

## Next session — start here

**D: claim another static document** (not Wikipedia / GitHub / mail).
Twenty-one public sites (libressl.org claimed). Candidates:
openbsdfoundation.org, or similar. Arch package builds; install is `sudo pacman
