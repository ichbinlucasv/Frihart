...
1. **This / next session — D.** Claim another static document (not
   Wikipedia / GitHub / mail). Twenty public claims; CSS leftovers
   done. Candidates: libressl.org, or similar.
...
6. **v0.2.0** when: named static list is claimed (20 already), worker
...
Twenty public sites (openssh.com claimed). Candidates: libressl.org,
or similar. Arch package builds; install is `sudo pacman
