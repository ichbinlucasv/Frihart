...
**RFC 5869** (HKDF), **openbsd.org**. Settings is
...
1. **This / next session — D.** Claim another static document (not
   Wikipedia / GitHub / mail). Nineteen public claims; CSS leftovers
   done. Candidates: openssh.com, libressl.org, or similar.
...
6. **v0.2.0** when: named static list is claimed (19 already), worker
...
X25519), RFC 5869 HTML (HKDF), openbsd.org.

## Next session — start here

**D: claim another static document** (not Wikipedia / GitHub / mail).
Nineteen public sites (openbsd.org claimed). Candidates: openssh.com,
libressl.org, or similar. Arch package builds; install is `sudo pacman
