...
(→ openssh.org), **libressl.org**, **openbsdfoundation.org**, **openbgpd.org**. Settings is
...
1. **This / next session — D.** Claim another static document (not
   Wikipedia / GitHub / mail). Twenty-three public claims; CSS leftovers
   done. Candidates: opensmtpd.org, or similar.
...
6. **v0.2.0** when: named static list is claimed (23 already), worker
...
(→ openssh.org), libressl.org, openbsdfoundation.org, openbgpd.org.

## Next session — start here

**D: claim another static document** (not Wikipedia / GitHub / mail).
Twenty-three public sites (openbgpd.org claimed). Candidates:
opensmtpd.org, or similar. Arch package builds; install is `sudo pacman
